...
- **AI 评分**：单条 / 批量触发 AI 评分，支持并发控制、强制重评、实时进度弹窗（进度条 / 当前学生 / 预计剩余 / 成功失败列表，进度落库、多 worker 生效）
- **审批评分**：教师审阅 AI 评分结果，可修改各项指标得分、添加评语、生成并编辑测评报告
- **成绩发布**：单条 / 批量发布成绩，支持撤回与重新评分
- **成绩总览**：班级学情分析、任务成绩（加权总分）、全班成绩导出 Excel
- **学生画像**：经「成绩总览 → 学生姓名」跳转进入，雷达图、班级对比、历史成绩、测评报告下载（Word / PDF）
...
> 成绩总览「任务成绩」表中的「加权总分」与学生画像「综合得分」同口径，均按上述公式计算；Excel「任务总览」学生行同值。**单个任务的班级平均分（汇总行）仍为算术平均，不参与权重计算。**
